...
const processListsss = ref([]);
const processListsssaaa = ref([]);
const testCaseData = ref([]);
const processListPageData = ref([]);
const processListPageDatasss = ref([]);
const currProcessListPage = ref([]);
const currProcessListPagesss = ref([]);
const currProcessListIndex = ref(0);
const currProcessListIndexsss = ref(0);
const processPageSize = 10;
const processPageSizesss = 1000;
// 转换分页数组
...
// 转换分页数组
const formatProcessPageablesss = (data) => {
  console.log(data)
  const len = data.length;
  let start = 0;
  let end = processPageSizesss;
  // 重置分页数组
  processListPageDatasss.value = [];
  while (end <= len) {
    processListPageDatasss.value.push(data.slice(start, end));
    start = end;
    end += processPageSizesss;
  }
  if (len % processPageSizesss) {
    processListPageDatasss.value.push(data.slice(start, len));
  }
  setCurrListPagesss();
};
...
const changeProcessListPagesss = (pageNum) => {
  currProcessListIndexsss.value = pageNum - 1;
  setCurrListPagesss();
};
...
const setCurrListPagesss = () => {
  currProcessListPagesss.value =
      processListPageDatasss.value[currProcessListIndexsss.value];
};
watch(
    processList,
    (newVal) => {
...
watch(
    processListsss,
    (newVal) => {
      formatProcessPageablesss(newVal);
    }, {
      immediate: true,
      deep: true,
    }
...
const getProcessListsss = () => {
  processListsss.value = [];
  terminalWebsocket.send(
      JSON.stringify({
        type: 'processListsss',
        path: input
...
const clearProcesssss = () => {
  // processListsss.value = [];
  terminalWebsocket.send(
      JSON.stringify({
        type: 'processListsssaaa',
        uuid: device,
        caseList: multipleSelection,
        path: input
      })
  );
};
const terminalWebsocketOnmessage = (message) => {
  switch (JSON.parse(message.data).msg) {
    ...
    case 'processListDetailsss': {
      console.log(JSON.parse(message.data).detail)
      const de = JSON.parse(message.data).detail;
      if (de && de !== null) {
        processListsss.value.push(de);
      }
      break;
    }
    case 'appListDetail': {
    ...
    >
      ...
      >
        ...
        >
          ...
          <el-tab-pane label="执行管理" name="run">
            <el-tabs stretch type="border-card">
              <el-tab-pane>
                <div style="text-align: center">
                  <el-button
                ...
                <!--                <Pageable-->
                <!--                    :is-page-set="false"-->
                <!--                    :total="processListsss.length"-->
                <!--                    :current-page="currProcessListIndexsss + 1"-->
                <!--                    :page-size="processPageSizesss"-->
                <!--                    @change="changeProcessListPagesss"-->
                <!--                ></Pageable>-->
              </el-tab-pane>
            </el-tabs>
          </el-tab-pane>
